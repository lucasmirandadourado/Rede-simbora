...
public class ReviewCaronaTest {
	...
	public void reviewTest() {
		...
		// Segundo Review 
		try {
			perfilCaroneiro.reviewVagaEmCarona("mark", "1", "bill", "não faltou");
		} catch (PerfilException e2) {
			fail();
		}
		...
		try {
			assertEquals("1", perfilCaroneiro.getAtributoPerfil("bill", "presenças em vagas de caronas"));
		} catch (PerfilException e) {
			e.printStackTrace();
			fail();
		}
	...
	public void possiveisErros() {
		...
		try {
			perfilCaroneiro.reviewVagaEmCarona("mark", "1", "bill", "não dou mais carona");
		} catch(PerfilException e) {
		...
		try {
			perfilCaroneiro.reviewVagaEmCarona("mark", "1", "vader", "não funcionou");
		} catch (PerfilException e) {
