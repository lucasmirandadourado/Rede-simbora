...
public class VisualizarPerfilTeste {
	...
	public void criarUsuario() {
		...
		try {
			assertEquals("Mark Zuckerberg",
					perfil.getAtributoPerfil("mark", "nome"));
		} catch (PerfilException e) {
			fail();
		}
		...
		try {
			assertEquals("Palo Alto, California",
					perfil.getAtributoPerfil("mark", "endereco"));
		} catch (PerfilException e) {
			fail();
		}

		try {
			assertEquals("mark@facebook.com",
					perfil.getAtributoPerfil("mark", "email"));
		} catch (PerfilException e) {
			fail();
		}
		...
		try {
			assertEquals("[]",
					perfil.getAtributoPerfil("mark", "historico de caronas"));
		} catch (PerfilException e) {
			fail();
		}

		try {
			assertEquals("[]", perfil.getAtributoPerfil("mark",
		...
		try {
			assertEquals("0", perfil.getAtributoPerfil("mark",
	...
	@Test
	public void errosPassagemParametros() {
